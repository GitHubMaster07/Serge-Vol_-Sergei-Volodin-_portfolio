<img width="2550" height="690" alt="github-header-banner" src="https://github.com/user-attachments/assets/7253a521-f3e8-4ce3-8202-628f3a727884" />
👋 Hi, I'm Sergei Volodin

**Senior Software Development Engineer in Test (SDET)** with 7+ years of experience in automation, data validation, and CI/CD integration. I specialize in building scalable test frameworks for UI, API, database, and mobile testing — with a strong focus on quality, performance, and continuous delivery.

---

## 🚀 Featured Projects

- 🔧 [UI/API/Database Automation Framework](https://github.com/SergeVol/ui-api-db-framework)  
  Scalable framework using Java, Selenium, Playwright, and Rest Assured with CI/CD integration.

- 📱 [Mobile Testing Framework (Appium)](https://github.com/SergeVol/mobile-testing-framework)  
  Mobile framework using Appium.

- 📄 [Test Strategy & Documentation Samples](https://github.com/SergeVol/test-docs)  
  Includes test plans, traceability matrices, defect reports, and BDD scenarios.

---

## 📫 Connect with Me

- [LinkedIn](https://linkedin.com/in/serge-vol)
- [Download My Resume](https://drive.google.com/file/d/1BPCWFH3ivWbwBRBizxpgeBrCY2_9OwPM/view?usp=sharing)

---

## 🧠 Certifications

### 🟦 Amazon Web Services (AWS)
- AWS Educate: Introduction to Generative AI
- AWS Lambda Foundation
- AWS SimuLearn: Cloud Practitioner
- AWS Educate: Machine Learning Foundations

### 🟪 IBM SkillsBuild
- Agile Explorer
- Big Data 101
- Big Data Foundations – Level 1 & 2
- Data Fundamentals
- Artificial Intelligence Fundamentals
- Cloud Computing Fundamentals
- Cybersecurity Fundamentals
- Hadoop 101
- Spark Fundamentals I
- Spark – Level 1
- Project Management Fundamentals
- Working in a Digital World: Professional Skills

### 🟨 Microsoft
- AI for Organizational Leaders
- Design a Machine Learning Solution

### 🟥 Google
- Google Cloud Data Analytics Certificate

### 🟧 LinkedIn Learning
- Critical Thinking for Better Judgment and Decision-Making

### 🟫 HIPAA Training US
- HIPAA Compliance Training Program (Valid through Oct 2026)

---

## 🧰 Tech Stack

Java · Selenium · Playwright · Appium · Rest Assured · TypeScript/JavaScript · PostgreSQL · Jenkins · Azure DevOps · CI/CD · ETL · GenAI Tools · SQL · JIRA · TestRail · Zephyr · Sauce Labs · BrowserStack · Git · IntelliJ · Linux

---

Thanks for visiting my portfolio! Feel free to explore my pinned projects or reach out for collaboration.
